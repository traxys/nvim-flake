...
with lib; let
  ...
  in
    ...
    ''"none"'' "";
in {
  options.plugins.clangd_extensions = {
    ...
    package = mkOption {
      ...
      description = "Plugin to use for clangd_extensions.nvim";
    };
  ...
  config = let
    cfg = config.plugins.clangd_extensions;
    setupOptions = {
  ...
  in
    mkIf cfg.enable {
      ...
      plugins.clangd_extensions.server.extraOptions = mkIf cfg.enableOffsetEncodingWorkaround {
