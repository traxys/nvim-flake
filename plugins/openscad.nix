...
in {
  options.plugins.openscad = {
    ...
    fuzzyFinder =
      helpers.defaultNullOpts.mkEnum defaultFuzzyFinder ["skim" "fzf"]
    ...
    keymaps = {
      enable = helpers.defaultNullOpts.mkBool false "keymaps for openscad";
  ...
  in
    mkIf cfg.enable {
      ...
      extraPackages = with pkgs; [zathura htop openscad];
