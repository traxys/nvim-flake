...
with lib; {
  options.plugins.gitmessenger = {
    ...
    closeOnCursorMoved = mkOption {
      type = types.bool;
      default = true;
      description = "A popup window is no longer closed automatically when moving a cursor after the window is shown up.";
    };
    includeDiff = mkOption {
      type = types.enum ["none" "current" "all"];
      default = "none";
      description = ''
    ...
    gitCommand = mkOption {
      type = types.str;
      default = "git";
      description = "git command to retrieve commit messages.";
    };
    noDefaultMappings = mkEnableOption "When this value is set, it does not define any key mappings";
    intoPopupAfterShow = mkOption {
      type = types.bool;
      default = true;
      description = ''
    ...
    concealWordDiffMarker = mkOption {
      type = types.bool;
      default = true;
      description = ''
  ...
  in
    mkIf cfg.enable {
      extraPlugins = with pkgs.vimPlugins; [git-messenger-vim];
      globals = {
